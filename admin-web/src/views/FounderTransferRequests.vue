<template>
  <div class="page-stack">
    <PageHeader title="创始人转让审核" description="仅 ROOT_ADMIN / SUPER_ADMIN 可审核，批准后家庭创始人立即变更。" />
    ...
    <el-alert v-if="loadError" :title="`转让申请列表加载失败：${loadError}`" type="error" show-icon :closable="false" />
    <el-alert v-if="operationError" :title="`操作失败：${operationError}`" type="error" show-icon @close="operationError = ''" />

    <DataTable>
      <el-table v-loading="loading" :data="items" stripe empty-text="暂无创始人转让申请">
        ...
        <el-table-column label="原创始人" min-width="160">
          <template #default="{ row }">
            <div class="id-pair">
              <span>成员：{{ row.fromMemberId }}</span>
              <span>用户：{{ row.fromUserId }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="目标创始人" min-width="160">
          <template #default="{ row }">
            <div class="id-pair">
              <span>成员：{{ row.toMemberId }}</span>
              <span>用户：{{ row.toUserId }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="170" fixed="right">
          <template #default="{ row }">
            <div v-if="row.requestStatus === 'PENDING'" class="table-actions">
              <el-button size="small" type="primary" @click="openAudit(row.requestId, 'approve')">通过</el-button>
              <el-button size="small" type="danger" @click="openAudit(row.requestId, 'reject')">驳回</el-button>
            </div>
    ...
    <ConfirmDialog
      ...
      title="确认转让创始人"
      message="批准后原创始人将降为普通成员，目标成员将立即成为唯一创始人。请确认已核对成员 ID 和用户 ID。"
      :submitting="submitting"
...
import {
  approveFounderTransferRequest,
  listFounderTransferRequests,
  rejectFounderTransferRequest
...
const items = ref<FounderTransferRequest[]>([])
const page = ref(1)
...
async function loadRequests() {
  ...
  loadError.value = ''
  try {
    const result = await listFounderTransferRequests({
...
function openAudit(requestId: number, action: 'approve' | 'reject') {
  ...
  auditTitle.value = `创始人转让申请 ${requestId}`
  auditKey.value += 1
...
async function submitAudit(reviewComment: string) {
  ...
  operationError.value = ''
  try {
    await rejectFounderTransferRequest(selectedRequestId.value, { reviewComment: comment })
    auditVisible.value = false
    ElMessage.success('创始人转让申请已驳回')
    await loadRequests()
  } catch (error) {
...
async function confirmApprove() {
  ...
  operationError.value = ''
  try {
    await approveFounderTransferRequest(selectedRequestId.value, {
      reviewComment: pendingReviewComment.value
    })
    confirmVisible.value = false
    ElMessage.success('创始人转让申请已通过')
    await loadRequests()
  } catch (error) {
    operationError.value = getApiErrorMessage(error)
  } finally {
